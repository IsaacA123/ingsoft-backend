...
class User {
  ...
  static async updateUser(id, { email, password }, code) {
    ...
    try {
        if (email) {
          const { error } = userSchema.fork(['password'], (schema) => schema.optional()).validate({ email });
          if (error) {
            throw new Error(`Error validando el correo: ${error.details[0].message}`);
          }
          return await db.execute(`UPDATE users SET email = ? WHERE id = ?`, [email, id]);        
        }

        if (password) {
          const { error } = userSchema.fork(['email'], (schema) => schema.optional()).validate({ password });
          if (error) {
            throw new Error(`Error validando la contraseña: ${error.details[0].message}`);
          }
